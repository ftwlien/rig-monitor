        ...
        if gpu_rows:
            ...
            for g in visible_gpu_rows:
                ...
                if wall_mode and not self.force_compact_gpu:
                    ...
                    util_label = pad_rich_right(f"UTIL [{gpu_color}]{g.util}%[/{gpu_color}]", 10)
                    mem_label = pad_rich_right(f"VRAM [{mem_color}]{g.mem_util_pct:.0f}%[/{mem_color}]", 10)
                    ...
                    junc_field = pad_rich_right(f"Junc [{junction_temp_color}]{g.junction_c}°C[/{junction_temp_color}]" if g.junction_c is not None else "", 18)
                    ...
                    fan_field = pad_rich_right(f"FAN {fan_value}", 21)
                    pwr_field = pad_rich_right(f"PWR [{pwr_color}]{g.power_w:.0f}W[/{pwr_color}]", 17)
